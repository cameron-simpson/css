        ...
        with Pfx(op):
            if op == 'url':
                if not argv:
                    ...
                    pipe_funcs, errors = argv_pipefuncs(
                        argv, P.action_map, P.do_trace)
                    ...
                    if not badopts:
                        ...
                        if cs.logutils.D_mode or ifdebug():
                            ...
                            def pinger(L):
                                while True:
                                    D("PINGER: L: quiescing=%s, state=%r: %s",
                                      L._quiescing, L._state, L)
                                    sleep(2)
...
class Pilfer(O):
  ...
  '''
    def __init__(self, *a, **kw):
        ...
        self.seensets = {}
        self.diversions_map = {
        }  # global mapping of names to divert: pipelines
    ...
    def _(self, value):
        if value is not None and not isinstance(value, str):
            raise TypeError("Pilfer._: expected string, received: %r" %
                            (value, ))
    ...
    '''
        ...
        if name not in seen:
            backing_path = MappingChain(
                mappings=[rc.seen_backing_paths for rc in self.rcs]).get(name)
            if backing_path is not None:
                ...
                if (not os.path.isabs(backing_path)
                    ...
                    if backing_basedir is not None:
                        ...
                        backing_path = os.path.join(backing_basedir,
                                                    backing_path)
    ...
    def quiesce_diversions(self):
        ...
        while True:
            ...
            for div in self.diversions:
                ...
                div.quiesce()
            D("%s.quiesce_diversions: now check that they are all quiet...",
              self)
            quiet = True
            for div in self.diversions:
                if div.counter:
        ...
        if pipe_name not in diversions:
            ...
            if errors:
                for err in errors:
                    error(err)
                raise KeyError(
                    "invalid pipe specification for diversion named %r" %
                    (pipe_name, ))
            name = "DIVERSION:%s" % (pipe_name, )
            outQ = NullQueue(name=name, blocking=True)
            outQ.open(
            )  # open outQ so it can be closed at the end of the pipeline
    ...
    '''
        if isinstance(pipe_name, PipeSpec):
            ...
            spec = self.pipes.get(pipe_name)
            if spec is None:
                raise ValueError("no pipe specification named %r" %
                                 (pipe_name, ))
        if name is None:
    ...
    '''
        debug("save_url(U=%r, saveas=%r, dir=%s, overwrite=%r, kw=%r)...", U,
              saveas, dir, overwrite, kw)
    ...
    def import_module_func(self, module_name, func_name):
        with LogExceptions():
            pylib = [
                path for path in envsub(self.defaults.get(
                    'pythonpath', '')).split(':') if path
            ]
            return import_module_name(module_name, func_name, pylib,
                                      self._lock)
    ...
    def __setitem__(self, k, value):
        ...
        with Pfx(url):
            if self._ok_attrkey(k):
                raise KeyError("it is forbidden to assign to attribute .%s" %
                               (k, ))
            else:
                P.user_vars[k] = value
...
def url_query(U, *a):
    ...
    if not a:
        return U.query
    qsmap = dict([(qsp.split('=', 1) if '=' in qsp else (qsp, ''))
...
def parse_action(action, do_trace):
  ...
  '''
    ...
    if action.startswith('.'):
        ...
        exts = exts.split(',')
        return FUNC_SELECTOR, lambda P: has_exts(
    ...
    if action.startswith('~', offset):
        ...
        if selector.sig != FUNC_SELECTOR:
            raise ValueError("expected selector function but found: %s" %
                             (selector, ))
    ...
    if name == 's':
        ...
        debug("s: regexp=%r, replacement=%r, repl_all=%s, repl_icase=%s",
              regexp, repl_format, repl_all, repl_icase)
    ...
    if name == 'grok' or name == 'grokall':
        ...
        if offset < len(action):
            raise ValueError("unparsed content after args: %r",
                             action[offset:])
        if name == "grok":
        ...
        '''
                if not isinstance(Ps, list):
                    Ps = list(Ps)
                if Ps:
                    mfunc = Ps[0].import_module_func(grok_module,
                                                     grok_funcname)
                    if mfunc is None:
                        error("import fails")
                    else:
    ...
    if name in ('see', 'seen', 'unseen'):
        ...
        if offset < len(action):
            ...
            if offset < len(action):
                if action[offset] != marker:
                    raise ValueError(
                        "parse should have a second marker %r at %r" %
                        (action[offset:]))
        ...
        if name == 'seen':
            ...
            def seen(P):
                ...
                see_value = P.format_string(value, U)
                return any(
                    [P.seen(see_value, seenset) for seenset in seensets])
...
def retriable(func):
  ...
  '''
    ...
    '''
        if not P.test_flags():
            raise RetryError('flag conjunction fails: %s' %
                             (' '.join(P.flagnames)))
        return func(P, *a, **kw)
  ...
  '''
    ...
    def _startproc(self, shcmd):
      ...
      '''
            for line in fp:
                if not line.endswith('\n'):
                    raise ValueError('premature EOF (missing newline): %r' %
                                     (line, ))
...
class PipeSpec(O):
  ...
  '''
    ...
    '''
        with Pfx(self.name):
            pipe_funcs, errors = argv_pipefuncs(self.argv, L, action_map,
                                                do_trace)
        return pipe_funcs, errors
            ...
            if cfg.has_section('pipes'):
                for pipe_name in cfg.options('pipes'):
                    with Pfx('[pipes].%s', pipe_name):
                        ...
                        self.add_pipespec(
                            PipeSpec(pipe_name, shlex.split(pipe_spec)))
