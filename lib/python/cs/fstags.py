...
class FSTagsCommand(BaseCommand, TagsCommandMixin):
  ...
  @uses_runstate
  def cmd_autotag(self, argv, *, runstate: RunState):
    ''' Usage: {cmd} paths...
  ...
  def cmd_edit(self, argv):
    ...
    with Pfx(path):
      if directories_like_files or not isdirpath(path):
        ...
        fstags[path].edit(comments=["", path, ""])
    ...
    for path in paths:
      with Pfx(path):
        ...
        for key, value in data.items():
          ...
          tagged_path.add(Tag(tag_name, value, ontology=ont), verbose=verbose)
    return 0
            ...
            with FSTagsTagFile(tagfilepath, fstags=fstags) as tagfile:
              ...
              for choice in tag_choices:
                with Pfx(choice.spec):
                  if choice.choice:
                    # add tag
                    tags.add(choice.tag, verbose=verbose)
                  else:
                    # delete tag
                    tags.discard(choice.tag, verbose=verbose)
        else:
...
class FSTags(MultiOpenMixin, Entities):
  ...
  def __init__(
    ...
    ''' Initialise the `FSTags` instance.
        ...
        * `physical`: optional flag for the associated `FSTagsConfig`
          ...
          filesystem paths
        * `tagsets`: an optional `BaseTagSets` to be used via the `Entities` methods
    ...
    if tagsets is None:
      ...
      Entities.__init__(self, tagsets=tagsets)
  ...
  def test(self, path, tag_tests, use_direct_tags=False):
    ...
    '''
    tagged_path = self[path]
    te = tagged_path if use_direct_tags else tagged_path.all_tags
  ...
  def edit_dirpath(self, dirpath, all_names=False):
    ...
    changed_tes = TagSet.edit_tagsets(te_map)
  ...
  def scrub(self, path):
    ...
    with Pfx("scrub %r", path):
      if isdirpath(path):
        ...
        for name in sorted(tagsets.keys()):
          with Pfx(name):
            ...
            if not existspath(subpath):
              vprint(f'{subpath}: does not exist, delete')
              del tagsets[name]
              modified = True
        if modified:
          tagfile.save()
        ...
        tagsets = tagfile.tagsets
        if base in tagsets:
          vprint(f'{path}: does not exist, deleted')
          del tagsets[base]
          tagfile.save()
