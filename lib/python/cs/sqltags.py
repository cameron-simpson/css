...
class SQLTagSet(SingletonMixin, TagSet):
  ...
  def deref(self, tag_name, attr=None, *, subtype=None):
    ''' Dereference the tag `tag_name`, returning the entities it 
        implies, using the tag value as the target entity key or keys.
        ...
        which is `{self.type_zone}.{subtype}.{key}`.
        ...
        Some examples should clarify. Suppose we have `self`
        ...
        Calling `self.deref('characters')` would return a list
        containing the entities named `tvdb.character.56`,
        `tvdb.character.123`, `tvdb.character.78`.

        Calling `self.deref('genres', 'genre_title')` would return
        a `dict` mapping `"Documentary"` and `"Food"` to a list of
        entities whose `.genre_title` matched each genre and whose
        `.name` started with `tvdb.genre.`.

        Calling `self.deref('studio')` would return the entity named `tvdb.studio.99`.

        Calling `self.deref('episode', 'summary')` would return a list
        of the entities whose `.summary` was `"Fred explores the foods of somewhere."`
        and whose `.name` started with `tvdb.episode`.
    '''
    try:
      value = self[tag_name]
    except KeyError:
    ...
    if isinstance(value, Sequence):
      ...
      if attr is None:
        # use the entity name
        return [sqltags[f'{zone}.{subtype}.{key}'] for key in value]
      # look up the attribute value
      deref_name_condition = f'name~{zone}.{subtype}.*'
      result = {
          key: list(trace(sqltags.find)(deref_name_condition, **{
              attr: key
          }))
          for key in value
      }
    ...
    if subtype is None:
      subtype = tag_name
    if attr is None:
      return sqltags[f'{zone}.{subtype}.{value}']
    return list(trace(sqltags.find)(deref_name_condition, **{attr: value}))
