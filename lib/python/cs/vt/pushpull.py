...
def missing_hashcodes(S1, S2, window_size=None, hashclass=None):
  ...
  while hashcodes2 and (hashcodes1 is None or hashcodes1):
    ...
    for ndx, hashcode in enumerate(hashcodes2):
      if hashcodes1 is None or hashcode > last_hashcode1:
        ...
        hashcodes1 = list(S1.hashcodes(start_hashcode=hashcode, hashclass=hashclass, length=window_size))
        if not hashcodes1:
    ...
    hashcodes2 = list(S2.hashcodes(start_hashcode=last_hashcode2, hashclass=hashclass, length=window_size,
                                   after=True))
  ...
  while True:
    ...
    hashcodes2 = S2.hashcodes(start_hashcode=hashcode, hashclass=hashclass, length=window_size, after=True)
...
def missing_hashcodes_by_checksum(S1, S2, window_size=None, hashclass=None):
  ...
  while True:
    ...
    hash1, h_final1 = S1.hash_of_hashcodes(
            length=window_size, start_hashcode=start_hashcode, hashclass=hashclass, after=after)
    if h_final1 is None:
    ...
    hashcodes2 = list(S2.hashcodes(start_hashcode=start_hashcode, hashclass=hashclass, length=window_size, after=after))
    if not hashcodes2:
      # maybe some entires removed? - anyway, no more S2 so return
      return
    hashcodes1 = set(S1.hashcodes(start_hashcode=start_hashcode, hashclass=hashclass, length=window_size, after=after))
    if not hashcodes1:
    ...
    for ndx, hashcode in enumerate(hashcodes2):
      ...
      if hashcodes1 and h_final1 < hashcode:
        ...
        hashcodes1 = set(S1.hashcodes(start_hashcode=hashcode, hashclass=hashclass, length=len(hashcodes2)-ndx))
        if hashcodes1:
          h_final1 = max(hashcodes1)
        else:
          h_final = None
