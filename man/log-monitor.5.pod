...
Each field in the clause specifies a test with a match rule and an action.
...
Matching lines are written to the shell command:
...
The former (B<ssh1>) appends report lines to the file B</var/log/sshd-lines>.
The latter (B<sshd2>) appends lines of the form:

  logfile matched-line
...
There should be a way to send syslog messages,
though the logger(1) command may be used with B<!> actions to achieve this.
(Maybe "@..." notation, as in syslogd.conf(5)).

=head1 SEE ALSO

log-monitor(1cs), tail(1), logger(1)
