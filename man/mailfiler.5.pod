...
Deliverable targets are those which deliver a message to some location such as a mail folder, email address or a shell pipeline.
Action targets affect the filer status or the message itself, such as a variable assignment or a message header modification.

The action targets are applied as they are encountered in matching rules. The deliverable targets are accrued for delivery at the end of the rules.
...
The parameter name is the header name in lowercase with dash ('-') replaced by underscore ('_').
