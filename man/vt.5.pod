...
the value is right shifted 7 places (a multiplication of 128)
and the 7 low order bits of the new byte are added to the value.

=head2 Hex Encoding

Raw byte sequences are sometimes encoded as hexadecimal sequences
where textual representation is required,
...
A data store managed by the cs.venti.gdbmstore.GDBStore class
